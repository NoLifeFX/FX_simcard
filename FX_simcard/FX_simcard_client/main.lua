...
RegisterNetEvent('kenzh1N:OpenSim')
AddEventHandler('kenzh1N:OpenSim', function()
	OpenPhoneMenu()
end)
...
function OpenSimMenu()
	...
	local elements2 = {}
	  ...
	  ESX.TriggerServerCallback('kenzh1N:GetList', function(sim)
		...
		local elements2 = {
		  ...
		  function(data2, menu2)
			...
			if data2.current.value == 'sim_use' then
				ESX.UI.Menu.CloseAll()
				TriggerServerEvent('kenzh1N:sim_use', data.current.value.number)
				ESX.ShowNotification("Carte SIM activée maintenant numéro ; ~o~" .. data.current.value.number)
			...
			if data2.current.value == 'sim_give' then
				...
				if closestPlayer == -1 or closestDistance > 3.0 then
					ESX.ShowNotification('Il n\'y a personne avec toi.')
				else
					TriggerServerEvent('kenzh1N:sim_give', data.current.value.number, GetPlayerServerId(closestPlayer))
				end
				TriggerServerEvent('gcPhone:allUpdate')
			end
			if data2.current.value == 'sim_delete' then
				ESX.UI.Menu.CloseAll()
				TriggerServerEvent('kenzh1N:sim_delete', data.current.value.number)
				ESX.ShowNotification("Vous avez détruit la carte SIM. Ce numéro est parti ~o~" .. data.current.value.number)
				TriggerServerEvent('gcPhone:allUpdate')
			end
	...
	ESX.UI.Menu.Open(
	  ...
	  function(data, menu)
		...
		if data.current.value == 'open_phone' then
			ESX.UI.Menu.CloseAll()
			ESX.TriggerServerCallback('kenzh1N:CheckSim', function(sim)
				if sim then
...
Print ("NOLIFEFX IS COME BACK BROOOO (' ")