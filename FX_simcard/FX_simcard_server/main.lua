		...
		MySQL.Async.fetchAll(
			...
			function(result)
				...
				MySQL.Async.execute(
					...
					function (rowsChanged)
					TriggerClientEvent('esx:showNotification', source, "Yeni bir sim kart aldın")
					end
				)
		end)
end

ESX.RegisterServerCallback('kenzh1N:CheckSim', function(source, cb)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
RegisterServerEvent('kenzh1N:sim_give')
AddEventHandler('kenzh1N:sim_give', function (store, index)
  local _source = source
  local id = index
  local xPlayer = ESX.GetPlayerFromId(_source)
  ...
  TriggerClientEvent('esx:showNotification', _source, "Sim kartı yakınındakine verdin numara artık ona ait ~o~" .. simZ)
  TriggerClientEvent('esx:showNotification', id, "Birisi sim kart uzattı numarası; ~o~" .. simZ)
...
RegisterServerEvent('kenzh1N:sim_delete')
AddEventHandler('kenzh1N:sim_delete', function (sim)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local sim = sim
...
RegisterServerEvent('kenzh1N:sim_use')
AddEventHandler('kenzh1N:sim_use', function (sim)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	--local new = sim
...
ESX.RegisterServerCallback('kenzh1N:GetList', function(source, cb)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local cartesim = {}
...
ESX.RegisterServerCallback('kenzh1N:GetNameZ', function(source, cb, id)
